...
// struct return{ to be added
    ...
    void motorSet(int motorNum, double Speed) {
        double MotorSpeed;
        if (motorNum == 3 && Speed > 0) { //right side forwards -0.98
            MotorSpeed = ((Speed * -0.85) / (100 / 17.5)) + 47.5;
        }
        if (motorNum == 5 && Speed > 0) { //left side forwards 0.55
            MotorSpeed = ((Speed * 0.65) / (100 / 17.5)) + 47.5;
        }
        if (motorNum == 3 && Speed < 0) { //right side backwards -1.0
            MotorSpeed = ((Speed * -1.0) / (100 / 17.5)) + 47.5;
        }
        if (motorNum == 5 && Speed < 0) {  //left side backwards 0.85
            MotorSpeed = ((Speed * 0.85) / (100 / 17.5)) + 47.5;
        }
        if (Speed == 0) {
    ...
    void turnAngle(double angle) { // runs the motors for the duration needed to turn an inputted angle
        double time = (fabs(angle)* angleTune) / SPEED; // calculates how long is needed, fabs is an absolute value function
        if (angle > 0){ // if angle is positive, turns right.
            motorSet(3, -SPEED);
            motorSet(5, SPEED);
        }
        if (angle < 0){// if angle is negative, turns left.
            motorSet(3, SPEED);
            motorSet(5, -SPEED);
        }
        hardware_exchange();
    ...
    bool isRed(){
        ...
        for (int row = 0 ; row < 30 ; row++) {
            for (int col = 0; col < 320; col++) {
                ...
                if (Alpha > 25 && Red > colourRatio * Blue && Red > colourRatio * Green){ //Alpha must be above 25 to not accidentally find red in black ie. R=2 G=1 B=1
                    redPix++;
                }
            }
        }
    ...
    int findPosition (int startRow, int endRow, int startColumn, int endColumn){
        ...
        for (int row = startRow ; row <= endRow ; row++) { //for each pixel in the line set:
            for (int col = startColumn; col <= endColumn; col++) {
                int currentPixel = (int)get_pixel(row, col, 3);

                if (currentPixel < minBlackValue){ //if the pixel Alpha value is below the threshold, then it is black.
                    minBlackValue = currentPixel;
                }
                if (currentPixel > maxBlackValue){ //if the pixel Alpha value is below the threshold, then it is black.
        ...
        for (int row = startRow ; row <= endRow ; row++) { //for each pixel in the line set:
            for (int col = startColumn; col <= endColumn; col++) {
                ...
                if ((int)get_pixel(row, col, 3) < threshold && maxBlackValue - minBlackValue > 50){ //if the pixel Alpha value is below the threshold, then it is black.
        ...
        for (int i=0; i < line.size(); i++){ // for each value in the line vector list:
        ...
        // cout << count << endl;
        if (count >= lineMin && count <= lineMax){ //if the number of black pixels is more than x, then a black line is present.
            return position;
        }else{
    ...
    void startUp() {
        cout<<endl<<"Start up in progress"<<endl;
        ...
        sleep1(2000);
        cout<<endl<<"Start up in complete"<<endl<<endl;
    }
    ...
    void follow_the_straight_line(int moveStep, double angleAdjust, double angleSensitivity, bool redArmed){
        ...
        while(true) {
            ...
            cout << "L " << leftPosition << "   F " << frontPosition << "   R " << rightPosition << "   B "
                 << bottomPosition << endl;
            if (isRed() && redArmed) { // if there is no line at top or red is present, more to next quadrant.
                cout << "red detected" << endl;
                break;
            } else if (frontPosition == 999 && rightPosition == 999 && leftPosition == 999 && bottomPosition == 999) {
                cout << "No lines Detected" << endl;
